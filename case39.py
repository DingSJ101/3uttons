from base import NewSolution
class mySolution(NewSolution):
    def __init__(self) -> None:
        init = [2,3,3,3,0,3,3,2]
        dest = [3,2,3,2,3,3,0,3] # bad ui in game : >|>|>>O> is the right destination
        controls = [4]
        super().__init__(init,dest,controls)
    def left(self,state,controls):
        if controls[0]!=0:
            idx = controls[0]
            state[idx-1],state[idx] = state[idx],state[idx-1]
            controls[0] = idx-1
        return state,controls

    def right(self,state,controls):
        if controls[0]!=7:
            idx = controls[0]
            state[idx],state[idx+1] = state[idx+1],state[idx]
            controls[0] = idx+1
        return state,controls
    def middle(self,state,controls):
        idx = controls[0]
        left = state[0:idx]
        right = state[idx+1:]
        try:
            left = [left[-1]]+left[0:-1]
        except:
            left = []
        try:
            right = right[1:]+[right[0]]
        except:
            right = []
        state = left + [0]+right
        return state,controls

if __name__ == '__main__':
    b = mySolution()
    step = b.init_step
    def render(step):
        res = ""
        for s in step.state:
            if s==0:
                res+='O'
            if s==1:
                res+='<'
            if s==2:
                res+='|'
            if s==3:
                res+='>'
        print(res)
    render(step)
    step = b.move(step,'m')
    render(step)
    # below solutions has the same output : >|>|>>O>
    c = mySolution()
    step = c.move(c.init_step,'rrmrmml')
    render(step)
    step = c.move(c.init_step,'mrrmm') # this should be a right answer but it wasn't 
    render(step)
    step = c.move(c.init_step,'rrmrmlm')
    render(step)

    
    a = mySolution()
    print(a.run(20))